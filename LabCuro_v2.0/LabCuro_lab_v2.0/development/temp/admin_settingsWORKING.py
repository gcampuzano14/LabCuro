...
def config(localconfig_dir):
    regex_json = {
                  'file_types' :{
                                 ...
                                 "COPATH" : {'add_to_clin_database': {'BONE MARROW | PERIPHERAL BLOOD':'(\D{1,2}\d{2}-{1}\d+\w{0,})\s+(\w+-{0,1}\w+)\s+((?:BM)|(?:PB))\s+(\w+\d*).+?(?=pdf)',
                                                                      'TISSUE | FLUIDS | OTHER':'(\D{1,2}\d{2}-{1}\d+\w{0,})\s+(\w+-{0,1}\w+)\s+((?:TS)|(?:FL))\s+(\w+\d*).+?(?=pdf)'},
                                             'in_processed_folder' : "(\D{1,2}\d{2}-{1}\d+\w{0,})\s+(\w+-{0,1}\w+)\s+((?:BM)|(?:PB)|(?:TS)|(?:FL))\s+(\w+\d*).+?(?=pdf)",
                                             'in_raw_folder' : "(\D{1,2}\d{2}-{1}\d+\w{0,})\s+(\w+-{0,1}\w+)\s.*?",
                                             'to_merge': "({{{_FILE_TO_MERGE_}}})(\D{1,2}\d{2}-{1}\d+\w{0,}\s+\w+-{0,1}\w+\s+(?:(?:BM)|(?:PB)|(?:TS)|(?:FL))\s+\w+\d*)(.+)(?=pdf)"},
                                 "SUNQUEST" : {'add_to_clin_database': {'BONE MARROW | PERIPHERAL BLOOD':'([MTXWFHS]\d+\w{0,})\s+(\w+-{0,1}\w+)\s+((?:BM)|(?:PB))\s(\w+\d*).+?(?=pdf)',
                                                                      'TISSUE | FLUIDS | OTHER':'([MTXWFHS]\d+\w{0,})\s+(\w+-{0,1}\w+)\s+((?:TS)|(?:FL))\s(\w+\d*).+?(?=pdf)'},
                                               'in_processed_folder': "([MTXWFHS]\d+\w{0,})\s+(\w+-{0,1}\w+)\s+((?:BM)|(?:PB)|(?:TS)|(?:FL))\s(\w+\d*).+?(?=pdf)",
