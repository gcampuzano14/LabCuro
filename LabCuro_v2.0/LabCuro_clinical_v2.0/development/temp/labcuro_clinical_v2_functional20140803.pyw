...
class MainWindow(QMainWindow, labcuroMainWindow.Ui_MainWindow, StartupGui):
    ...
    def startdialog(self):
        ...
        netdrives = self.network_share_auth(username, password, domain, shares)
        structure = self.file_architect(netdrives, username)
        return structure, username, poweruser, poweruser_list

    def network_share_auth(self, username, password, domain, shares):
        drive_letters = []
        for netdrive in shares:
            try:
                letter = map_network_drive.map_netdrive(netdrive, domain, username, password)
            #if letter:
